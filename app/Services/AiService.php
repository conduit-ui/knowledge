...
class AiService
{
    ...
    public function isAvailable(): bool
    {
        ...
        $provider = config('search.ai.provider', 'ollama');

        if ($provider === 'openrouter') {
            return ! empty(config('search.ai.openrouter.key'));
        }
    ...
    {
        $key = config('search.ai.openrouter.key');
        $model = config('search.ai.openrouter.model', 'anthropic/claude-3.5-sonnet');
        ...
        try {
            ...
            if ($response->getStatusCode() !== 200) {
                echo "OpenRouter Error: Status " . $response->getStatusCode() . "\n";
                echo "Response: " . (string) $response->getBody() . "\n";
                return '';
            }

            $body = (string) $response->getBody();
            // echo "OpenRouter Response: " . substr($body, 0, 500) . "\n"; // Debug

            $data = json_decode($body, true);

            if (! is_array($data) || ! isset($data['choices'][0]['message']['content'])) {
                echo "OpenRouter Invalid JSON or Structure: " . substr($body, 0, 200) . "\n";
                return '';
            }
    ...
    protected function generateOllama(string $prompt): string
    {
